...
function CourseHome({ version = 'user' }: HomeProps) {
  ...
  const recommendedTopics = useMemo(() => getRecommendedTopics(course, placementResult), [course, placementResult]);
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        {firstPlacementTest && (
          <section className="mb-12 bg-white border border-blue-100 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
              <div>
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-sm mb-3">
                  ...
                  Персональный порядок тем
                </div>
                <h2 className="text-2xl font-bold mb-2">
                  {placementResult ? 'Темы отсортированы по результатам входного теста' : 'Пройдите входной тест для персональных рекомендаций'}
                </h2>
                <p className="text-gray-600 max-w-3xl">
                  Каждый ответ добавляет скор к одному из модулей. Модули и предложенные темы с большим скором поднимаются выше для ученика.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  <Sparkles className="w-4 h-4" />
                  {placementResult ? 'Пройти заново' : 'Начать тест'}
                </button>
                {placementResult && (
                  <button
                    type="button"
            ...
            {placementResult && recommendedTopics.length > 0 && (
              <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-3">
                {recommendedTopics.slice(0, 3).map(({ module, topic, score }, index) => (
                  <Link
                    key={`${module.id}-${topic.id}`}
                    to={`${routePrefix}/module/${module.id}/topic/${topic.id}`}
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-500 mt-1">{module.title}</div>
                  </Link>
                ))}
              </div>
            )}
        ...
        <div className="space-y-10">
          {sortedModules.map((module) => (
            <section key={module.id} className="bg-white border border-gray-200 rounded-3xl p-6 md:p-8 shadow-sm">
