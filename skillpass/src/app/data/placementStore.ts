import { Course, Module, PlacementTest } from './courseData';

const PLACEMENT_RESULT_KEY = 'course-placement-result-v3';
const PLACEMENT_DISMISSED_KEY = 'course-placement-dismissed-v3';
...
export function getRecommendedModules(course: Course, result: PlacementResult | null): RecommendedModule[] {
  return getSortedModules(course, result).map((module) => ({
    module,
    score: result?.scoresByModuleId[module.id] ?? 0,
  }));
}
